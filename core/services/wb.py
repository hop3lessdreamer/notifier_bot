...
class WbProductImgService:
    ...
    async def _request_to_get_img(self, product_id: int) -> bytes | None:
        url: str = self._get_img_url(product_id)
        loguru_logger.info(f'request ("{url}") to download img' f' by product = {product_id}.')
    ...
    async def _request_to_get_img_by_alt_url(self, product_id: int) -> bytes | None:
        ...
        async with ClientSession(
            ...
            if response.status >= HTTPStatus.BAD_REQUEST:
                loguru_logger.error(f'Не удалось получить изображения товара "{product_id}".')
                return None
            return cast(bytes, await response.read())
...
class WbProductInfoService:
    ...
    async def _get_raw_products_info(self, product_ids: list[ProductID]) -> dict[str, Any]:
        ...
        loguru_logger.info(f'request ("{url}") for products info ({product_ids})')
        async with ClientSession(
            timeout=ClientTimeout(total=DEFAULT_TIMEOUT), headers=get_fake_headers()
        ) as session, session.get(url) as response:
...
class WbService:
    ...
    async def get_products(self, product_ids: list[ProductID]) -> list[Product]:
        ...
        loguru_logger.info(f'prod_data = {prod_data}')
        return [
    ...
    async def try_get_product(self, product_id: ProductID) -> Product | None:
        ...
        try:
            ...
            loguru_logger.error(f'Ошибка при получении товара ({ex})!')
        return product
