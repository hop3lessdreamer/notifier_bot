...
class Messages:
    ...
    def current_product_price_w_exist_subscription_w_thr(subscription: Subscription) -> str:
        return fmt.text(
            ...
            f'{str(subscription.user_product.price_threshold.quantize(Decimal("1.00")))}.',
            sep=''
        )
    ...
    def product_added(product_title: str) -> str:
        return fmt.text(
            'Товар ', fmt.hbold(f'"{product_title}"'),
    ...
    def product_added_yet_wo_threshold(product_id: int) -> str:
        return fmt.text(
            'Товар ', fmt.hlink(str(product_id), form_url_from_product_id(product_id)),
            ' уже добавлен в отслеживаемые.',
            ' Бот уведомит вас при снижении цены.', sep=''
        )

    @staticmethod
    def product_added_w_threshold(product: WbProduct, threshold: Decimal) -> str:
        return fmt.text(
            'Товар ', fmt.hbold(f'"{product.title}"'),
            ' добавлен в отслеживаемые.',
    ...
    def product_added_yet_w_threshold(product_id: int, threshold: Decimal) -> str:
        return fmt.text(
            'Товар ', fmt.hlink(str(product_id), form_url_from_product_id(product_id)),
            ' уже добавлен в отслеживаемые.',
            f' Бот уведомит вас при снижении цены до значения - {str(threshold)}.', sep=''
        )

    @staticmethod
    def product_added_w_threshold_in_percent(
            wb_product: WbProduct,
            price_threshold: float | Decimal
    ) -> str:
        return fmt.text(
            'Товар ', fmt.hbold(f'"{wb_product.title}"'),
            ' добавлен в отслеживаемые.',
            f'\nБот уведомит вас при снижении цены до значения -'
            f' {str(price_threshold)}.',
            sep=''
        )
    ...
    def subscription(subs_info: SubscriptionsInfo) -> str:
        ...
        if sub.user_product.price_threshold == sub.product.price:
            return fmt.text(
                ...
                ' - ',
                f'уведомление придет при снижении цены '
                f'{str(sub.product.price)}',
                sep=''
            )
    ...
    def subscriptions_not_found() -> str:
        return fmt.text('У вас нет отслеживаемых товаров.', sep='')

    @staticmethod
    def subscriptions_not_found_after_deletion() -> str:
        return fmt.text('У вас не осталось отслеживаемых товаром после удаления.', sep='')

    @staticmethod
    def info_about_deletion(deleted_sub: Subscription) -> str:
        return fmt.text(
            'Товар ', fmt.hbold(f'"{deleted_sub.product.title}"'), ' удален из отслеживаемых.',
            sep=''
        )